    ...
    <tr style="font-size: 11px; padding: 5px;">
        @foreach ($chunk as $product)
            ...
            <td style="padding-top: 0px; padding-bottom: 0px;" class="product-img sound-btn filter_product_add"
                ...
                title="{{ $product->name }}" data-product="{{ $product->name . ' (' . $product->variation_name . ')' }}">
                ...
                <p><span
                    ...
                    @if (!empty($currency))
                        <span {{--                            style="font-size:12px !important; font-weight: bold; color: black;">{{ @num_format($default_sell_price / $exchange_rate) . ' ' . $currency->symbol }} --}}
                            style="font-size:12px !important; font-weight: bold; color: black;">{{ @num_format($default_sell_price / $exchange_rate) }}
                            {{ ' ' . $currency->symbol }}
                        </span>
                    @else
                        <span
